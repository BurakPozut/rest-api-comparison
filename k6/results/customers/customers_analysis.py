...
node_files = sorted(base_path.glob(f"{platform_folder}/customers-test-node-*.csv"))
asp_files = sorted(base_path.glob(f"{platform_folder}/customers-test-asp-*.csv"))
...
print("\n--- Summary Statistics (/api/customers) ---")
...
# Labels and title
ax1.set_title(
    f"Latency Over Time - /api/customers (100 VUs, 5 Run Avg) - Test Environment: {platform_folder}",
    fontsize=14
)
...
# Finalize
plt.tight_layout()
plt.savefig(f"results/customers/{platform_folder}/customers-latency-detailed-{platform_folder}.png", dpi=300)
plt.show()