...
node_files = sorted(base_path.glob(f"{platform_folder}/login-test-node-*.csv"))
asp_files = sorted(base_path.glob(f"{platform_folder}/login-test-asp-*.csv"))
...
# Labels and title
ax1.set_title(
    f"Latency Over Time - /api/login (100 VUs, 5 Run Avg) - Test Environment: {platform_folder}",
    fontsize=14
)
...
# Finalize
plt.tight_layout()
plt.savefig(f"results/login/{platform_folder}/login-latency-detailed-{platform_folder}.png", dpi=300)
plt.show()