...
node_files = sorted(base_path.glob(f"{platform_folder}/cpu-test-node-*.csv"))
asp_files = sorted(base_path.glob(f"{platform_folder}/cpu-test-asp-*.csv"))
...
# Labels and title
ax1.set_title(
    f"Latency Over Time - /api/cpu (100 VUs, 5 Run Avg) - Test Environment: {platform_folder}",
    fontsize=14
)
...
# Finalize
plt.tight_layout()
plt.savefig(f"results/cpu/{platform_folder}/cpu-latency-detailed-{platform_folder}.png", dpi=300)
plt.show()