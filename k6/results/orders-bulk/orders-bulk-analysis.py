...
node_files = sorted(base_path.glob(f"{platform_folder}/orders-bulk-test-node-*.csv"))
asp_files = sorted(base_path.glob(f"{platform_folder}/orders-bulk-test-asp-*.csv"))
...
print("\n--- Summary Statistics (/api/orders-bulk) ---")
...
# Labels and title
ax1.set_title(
    f"Latency Over Time - /api/orders-bulk (100 VUs, 5 Run Avg) - Test Environment: {platform_folder}",
    fontsize=14
)
...
# Finalize
plt.tight_layout()
plt.savefig(f"results/orders-bulk/{platform_folder}/orders-bulk-latency-detailed-{platform_folder}.png", dpi=300)
plt.show()