...
node_files = sorted(base_path.glob(f"{platform_folder}/file-read-test-node-*.csv"))
asp_files = sorted(base_path.glob(f"{platform_folder}/file-read-test-asp-*.csv"))
...
print("\n--- Summary Statistics (/api/file-read) ---")
...
# Labels and title
ax1.set_title(
    f"Latency Over Time - /api/file-read (100 VUs, 5 Run Avg) - Test Environment: {platform_folder}",
    fontsize=14
)
...
# Finalize
plt.tight_layout()
plt.savefig(f"results/file-read/{platform_folder}/file-read-latency-detailed-{platform_folder}.png", dpi=300)
plt.show()